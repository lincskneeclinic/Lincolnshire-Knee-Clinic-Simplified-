...
export function DiagnosticsClient() {
  ...
  const [requestedScan, setRequestedScan] = useState("MRI One Part Scan (£325)");
    ...
    <>
      ...
      <div className="my-12 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          {[
            { id: "mri", label: "🧲 MRI Scan", price: "From £325" },
            { id: "ultrasound", label: "🔊 Ultrasound", price: "£150" },
            { id: "xray", label: "🦴 Digital X-Ray", price: "£95" },
            { id: "ct", label: "🖥️ CT Scan", price: "From £350" },
            { id: "arthrogram", label: "💉 MR Arthrogram", price: "£600" },
          ].map((tab) => (
            <button
              key={tab.id}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="bg-pale-clinical-blue border border-clinical-teal/30 p-3 rounded-xl text-right">
                  ...
                  <span className="font-mono text-base font-bold text-deep-navy">From £325 <span className="text-xs text-text-muted font-normal">(One Part)</span></span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 text-xs">
                <div className="bg-soft-blue p-3.5 rounded-xl border border-border-clinical/30">
                  <span className="font-bold text-deep-navy block mb-1">MRI One Part Scan:</span>
                  <span className="font-mono font-bold text-clinical-teal text-sm">£325</span>
                </div>
                <div className="bg-soft-blue p-3.5 rounded-xl border border-border-clinical/30">
                  <span className="font-bold text-deep-navy block mb-1">MRI Two Part Scan:</span>
                  <span className="font-mono font-bold text-clinical-teal text-sm">£525</span>
                </div>
              </div>
            </div>
          )}
          ...
          {selectedModality === "ultrasound" && (
            <div className="space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="bg-pale-clinical-blue border border-clinical-teal/30 p-3 rounded-xl text-right">
                  <span className="text-[10px] uppercase font-bold text-text-muted block">Vista Health Price</span>
                  <span className="font-mono text-base font-bold text-deep-navy">£150 <span className="text-xs text-text-muted font-normal">per area</span></span>
                </div>
              </div>
              <p className="text-sm text-text-secondary leading-relaxed">
          ...
          {selectedModality === "xray" && (
            <div className="space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="bg-pale-clinical-blue border border-clinical-teal/30 p-3 rounded-xl text-right">
                  <span className="text-[10px] uppercase font-bold text-text-muted block">Vista Health Price</span>
                  <span className="font-mono text-base font-bold text-deep-navy">£95 <span className="text-xs text-text-muted font-normal">per area</span></span>
                </div>
              </div>
              <p className="text-sm text-text-secondary leading-relaxed">
          ...
          {selectedModality === "ct" && (
            <div className="space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="bg-pale-clinical-blue border border-clinical-teal/30 p-3 rounded-xl text-right">
                  <span className="text-[10px] uppercase font-bold text-text-muted block">Vista Health Price</span>
                  <span className="font-mono text-base font-bold text-deep-navy">£350 <span className="text-xs text-text-muted font-normal">(One Part)</span> / £475 <span className="text-xs text-text-muted font-normal">(Two Part)</span></span>
                </div>
              </div>
              <p className="text-sm text-text-secondary leading-relaxed">
          ...
          {selectedModality === "arthrogram" && (
            <div className="space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-4">
                ...
                <div className="bg-pale-clinical-blue border border-clinical-teal/30 p-3 rounded-xl text-right">
                  <span className="text-[10px] uppercase font-bold text-text-muted block">Vista Health Price</span>
                  <span className="font-mono text-base font-bold text-deep-navy">£600 <span className="text-xs text-text-muted font-normal">(Guided)</span></span>
                </div>
              </div>
              <p className="text-sm text-text-secondary leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3.5 py-2.5 border border-border-clinical rounded-xl bg-slate-50 focus:outline-none focus:border-clinical-teal"
                      >
                        <option value="MRI One Part Scan (£325)">MRI One Part Scan (£325)</option>
                        <option value="MRI Two Part Scan (£525)">MRI Two Part Scan (£525)</option>
                        <option value="MR Arthrogram (£600)">MR Arthrogram (£600)</option>
                        <option value="Dynamic Ultrasound (£150)">Dynamic Ultrasound (£150)</option>
                        <option value="Digital X-Ray (£95)">Digital X-Ray (£95)</option>
                        <option value="CT Scan One Part (£350)">CT Scan One Part (£350)</option>
                        <option value="CT Scan Two Part (£475)">CT Scan Two Part (£475)</option>
                      </select>
                    </div>
                    <div>
