...
import { getRemovedArticleSlugs, getArticleOverrides, ArticleOverride, getDynamicArticles } from "@/lib/educationArticles";
...
function buildCategoriesData(
  ...
  dynamicArticles: Record<string, any> = {}
  ...
  Object.entries(categoryMeta).forEach(([catKey, meta]) => {
    const articlesInCat = Object.values(allArticles)
...
export default async function CategoryPage({ params }: PageProps) {
  const { category } = await params;
  const [removedSlugs, overrides, dynamicArticles] = await Promise.all([
    getRemovedArticleSlugs(),
    getArticleOverrides(),
    getDynamicArticles(),
  ]);
  const categoriesData = buildCategoriesData(removedSlugs, overrides, dynamicArticles);
