...
export const UrgentAdviceBanner: React.FC<UrgentAdviceBannerProps> = ({
  ...
  return (
    <div
      ...
      className={`bg-status-error-bg border-b border-[#FAD8D8] text-status-error px-4 py-2.5 font-sans text-sm md:text-[15px] leading-relaxed ${className}`}
    >
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-1.5">
        <div className="flex items-start md:items-center gap-2">
          <svg
            className="w-5 h-5 shrink-0 text-status-error mt-0.5 md:mt-0"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2.5}
